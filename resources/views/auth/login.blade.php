<x-guest-layout>
    ...
    <form method="POST" action="{{ route('login') }}" class="bg-white px-6 pb-6 pt-2 rounded-b">
        @csrf

        <!-- Email Address -->
        <div>
            {{-- <x-input-label for="email" :value="__('Email')" /> --}}
            <x-text-input id="email" placeholder="Email" class="block mt-1 w-full text-base" type="email" name="email" :value="old('email')" required autofocus autocomplete="username" />
